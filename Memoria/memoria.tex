...
\begin{figure*}[htp]
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[height=1.2in]{Mp}
	...
	\begin{subfigure}[t]{0.5\textwidth}
		\centering
		\includegraphics[height=1.2in]{ts}
...
En el diagrama de Bode se han marcado de forma aproximada los dos polos.
...
... margen de fase 30\degree (explicación)?? ... además, para que sea necesario el sistema completo PID, se desea que el error frente a una perturbación constante sea nulo.
...
con $\omega_0 = ?? rad/s$ y se va a desplazar al punto $B=G_C(j\omega_0) G(j\omega_0)$, que también corta la circunferencia unidad pero en el punto donde $M.F. = 30\degree$.
...
\begin{equation}
eq Ziegler-Nichols PID
\end{equation}

Si se elige aleatoriamente una $\alpha = ??$ se obtienen los siguientes parámetros:
...
Cuyo diagrama de Nyquist y respuesta al escalon se puede ver en la figura \ref{fig:sim} y se pueden comprobar que $M_P=??$ y $t_s=??$, cumpliendo las especificaciones de diseño. Además se obtiene el $M.F. = 30\degree$ deseado y un margen de ganancia infinito.

FIGURA CON DIAGRAMA DE NYQUIST DE Gc*G Y step

\subsection{Prueba del sistema diseñado en Telelabo}

Igual que en el otro ejercicio se calculan KP, KI y KD y se meten las imágenes

Se pone tambien la respuesta a un seno de w0 para ver la fase
